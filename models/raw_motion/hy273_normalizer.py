...
class HY273Normalizer:
    ...
    @classmethod
    def from_data_root(cls, data_root: str | Path, eps: float = 1e-6) -> "HY273Normalizer":
        root = Path(data_root)
    ...
    def to(self, device: torch.device | str, dtype: Optional[torch.dtype] = None) -> "HY273Normalizer":
        other = HY273Normalizer(self.mean.squeeze(0).squeeze(0), self.std.squeeze(0).squeeze(0))
